...
let catTxtMeasure = {};
let txtWidth = [];
const Festivals = ({ navigation, designStore, userStore }) => {
      ...
      >
        <View>
          <FlatList
            ...
            renderItem={({ item, index }) => {
              return (
                <TouchableOpacity onPress={() => onPressCat(item.name, index)}>
...
export default inject("designStore", "userStore")(observer(Festivals));
...
const styles = StyleSheet.create({
  ...
  listSubCategoryDesign: { backgroundColor: Color.white },
  imgAllDesign: {
    width: 150,
    height: 150,
    marginLeft: 10,
